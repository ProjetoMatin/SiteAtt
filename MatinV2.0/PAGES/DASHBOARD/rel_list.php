...
$limite = 4; // Número de arquivos por página
$inicio = ($pagina * $limite) - $limite;

// Conectando ao banco de dados com PDO
try {
    $conn = new PDO("mysql:host=localhost;dbname=matin", "root", "");
    $conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
} catch (PDOException $e) {
    die("Erro de conexão: " . $e->getMessage());
}

$idUsuario = $_SESSION['idUsu']; // Você deve garantir que o ID do usuário esteja na sessão
$limiteRelatorios = 5; // Limite de relatórios para usuários não premium

// Consulta para contar os relatórios gerados pelo usuário
$query = "SELECT COUNT(*) as totalRelatorios FROM relatorios WHERE idUsu = :idUsu";
$stmt = $conn->prepare($query);
$stmt->bindParam(':idUsu', $idUsuario, PDO::PARAM_INT);
$stmt->execute();
$totalRelatorios = $stmt->fetch(PDO::FETCH_ASSOC)['totalRelatorios'];

// Verifica se o usuário é premium ou não
$queryPremium = "SELECT premium FROM usuario WHERE idUsu = :idUsu";
$stmtPremium = $conn->prepare($queryPremium);
...
if ($totalRelatorios >= $limiteRelatorios && $isPremium == '0') {
...
<div class="conteudo" id="conteudo2">
    ...
    <div class="main-cont">
        <div class="main-tit">
            ...
            <h4>Lista de Relatórios Gerados (<?php echo $totalRelatorios; ?>/5)</h4>
        </div>
    </div>

    <?php if (!$podeGerarRelatorio): ?>
        <p class="txtVermelho"><?php echo $mensagemErro; ?></p>
    <?php else: ?>
        <!-- Código para gerar relatório aqui -->
        <div class="slot">
            <?php
            if (is_dir($directory)) {
                ...
                $totalArquivos = count($pdf_files);
                $totalPaginas = ceil($totalArquivos / $limite);
                $pdf_files = array_slice($pdf_files, $inicio, $limite);

                if (!empty($pdf_files)) {
                    foreach ($pdf_files as $pdf) {
                        $nomeNovo = filtrarNome($pdf);
                        $data = filtrarData($pdf);
                        $letraMaiuscula = ucfirst($nomeNovo);
                        echo "<div class='rel'>";
                        echo "<h6>Relatório de $letraMaiuscula</h6>";
                        echo "<p class='apagado'>Nome arquivo: $pdf</p>";
                        echo "<div class='flex-dir-esq'>";
                        echo "<p>Data emissão: $data</p>";
                        echo "</div>";
                        echo "</div>";
                    }
                } else {
                    echo "<p class='txtVermelho'>Nenhum arquivo PDF encontrado.</p>";
                }
            } else {
                echo "<p class='txtVermelho'>Pasta não encontrada.</p>";
            }
            ?>
        </div>
    <?php endif; ?>
